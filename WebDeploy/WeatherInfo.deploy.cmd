...
set _DeploySetParametersFile=%RootPath%WeatherInfo.SetParameters.xml
)
...
@rem ---------------------------------------------------------------------------------
if not exist "%RootPath%WeatherInfo.zip" (
echo "%RootPath%WeatherInfo.zip" does not exist. 
...
set _MSDeployCommandline="%MSDeployPath%msdeploy.exe" -source:package='%RootPath%WeatherInfo.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -skip:objectname='dirPath',absolutepath='obj\\Release\\Package\\PackageTmp\\App_Data$' -skip:objectname='dirPath',absolutepath='Default\ Web\ Site/WeatherInfo_deploy\\App_Data$'
) else (
set _MSDeployCommandline="%MSDeployPath%msdeploy.exe" -source:package='%RootPath%WeatherInfo.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -skip:objectname='dirPath',absolutepath='obj\\Release\\Package\\PackageTmp\\App_Data$' -skip:objectname='dirPath',absolutepath='Default\ Web\ Site/WeatherInfo_deploy\\App_Data$' -setParamFile:"%_DeploySetParametersFile%"
)
...
:usage
echo =========================================================
if not exist "%RootPath%WeatherInfo.deploy-readme.txt" (
...
start notepad "%RootPath%WeatherInfo.deploy-readme.txt"
)
echo =========================================================
goto :eof
